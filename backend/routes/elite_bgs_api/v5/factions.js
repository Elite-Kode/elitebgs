...
router.get('/', cors(), async (req, res, next) => {
    ...
    const factions = await redisGet(urlHash)
    if (factions != null) {
        res.status(200).send(JSON.parse(factions));
    } else {
        // Not cached, let's do it the hard way and stash the result in Redis
        try {
            let query = {};
            let page = 1;
            let history = false;
            ...
            if (history) {
                ...
                // Store the result in redis for up to 10 minutes (it will expire after that)
