...
async function getSystems(query, history, minimal, page, request) {
  ...
  if (
    request.query.activeState ||
    request.query.pendingState ||
    request.query.recoveringState ||
    request.query.influenceGT ||
    request.query.influenceLT ||
    request.query.factionAllegiance ||
    request.query.factionGovernment ||
  ...
  aggregate.addFields({
    factions: {
      $map: {
        ...
        as: 'faction_list',
        in: {
          $mergeObjects: ['$$faction_list', objectToMerge]
        }
      }
    }
  })
