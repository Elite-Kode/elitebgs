...
router.get('/factions/state', async (req, res, next) => {
  try {
    ...
    const series = []
    const states = Object.keys(FDevIDs.state)
      .filter((state) => {
        return state !== 'null'
      })
      .map((state) => {
...
let factionActivePendingRecovering = async (req, res, next, type) => {
  try {
    ...
    let highchartsRequestOptions = {
      url: `http://localhost:7801/`,
      formData: {
        options: un_eval(options)
          .replace('systemsRegex', JSON.stringify(systems))
          .replace('tickPositionsRegex', JSON.stringify(tickPositions))
          .replace('maxStatesConcurrentRegex', JSON.stringify(maxStatesConcurrent)),
        type: 'image/png',
    ...
    allTimeFactions.forEach((faction) => {
      ...
      factionHistory.forEach((record) => {
        if (record.faction_name === faction) {
          ...
          if (
            indexInSystem !== -1 &&
            history[indexInSystem].factions.findIndex((element) => {
...
let systemActivePendingRecovering = async (req, res, next, type) => {
  try {
    ...
    let highchartsRequestOptions = {
      ...
      formData: {
        options: un_eval(options)
          .replace('factionsRegex', JSON.stringify(factions))
          .replace('tickPositionsRegex', JSON.stringify(tickPositions))
          .replace('maxStatesConcurrentRegex', JSON.stringify(maxStatesConcurrent)),
        type: 'image/png',
        filename: `${req.query.name}-${req.query.timeMin}-${req.query.timeMax}-${stateType}`,
