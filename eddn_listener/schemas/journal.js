...
function Journal() {
      ...
      if (message.event === 'FSDJump' || message.event === 'Location') {
        if (message.Factions) {
          ...
          for (let faction of message.Factions) {
            ...
            ebgsFactionsModel
              .findOneAndUpdate({ name: faction.Name }, factionObject, {
                upsert: true,
                runValidators: true
              })
              .exec()
          ...
          ebgsSystemsModel
            .findOneAndUpdate({ name: systemObject.name }, systemObject, {
              upsert: true,
              runValidators: true
            })
            .exec()
    ...
    try {
      if (message.event === 'FSDJump' || message.event === 'Location') {
        if (message.Factions && this.checkMessage(message)) {
          ...
          ;(async () => {
            try {
              let system = await ebgsSystemsV3Model
                .findOne(
                  {
                    name_lower: message.StarSystem.toLowerCase(),
              ...
              if (!_.isEmpty(systemObject)) {
                ...
                if (hasEddbId) {
                  ...
                  try {
                    ...
                    // If eddb id cannot be fetched, create the record without it.
                    try {
                      await ebgsSystemsV3Model
                        .findOneAndUpdate(
                          {
          ...
          ;(async () => {
            try {
              ...
              let allFactionsPresentInSystem = ebgsFactionsV3Model
                .find(
                  {
                    faction_presence: {
              ...
              let allFactionsPresentInMessage = ebgsFactionsV3Model
                .find(
                  {
                    name_lower: {
              ...
              for (let dbFaction of factionsAllDetails) {
                for (let messageFaction of message.Factions) {
                  if (
                    messageFaction.Name.toLowerCase() === dbFaction.name_lower &&
                    dbFaction.updated_at < new Date(message.timestamp)
                  ) {
                    let pendingStates = []
                    if (messageFaction.PendingStates) {
                      messageFaction.PendingStates.forEach((pendingState) => {
                    ...
                    dbFaction.faction_presence.forEach((faction) => {
                      if (
                        faction.system_name_lower === message.StarSystem.toLowerCase() &&
  ...
  this.trackSystemV4 = async (message, header) => {
    if (message.event === 'FSDJump' || message.event === 'Location' || message.event === 'CarrierJump') {
      try {
        ...
        ;(async () => {
          try {
            let system = await ebgsSystemsV4Model
              .findOne({
                name_lower: message.StarSystem.toLowerCase(),
                x: this.correctCoordinates(message.StarPos[0]),
                y: this.correctCoordinates(message.StarPos[1]),
            ...
            if (system) {
              ...
              if (system.updated_at < new Date(message.timestamp)) {
                ...
                if (
                  ...
                  let systemHistory = await ebgsHistorySystemV4Model
                    .find({
        ...
        ;(async () => {
          try {
            ...
            let allFactionsPresentInSystem = ebgsFactionsV4Model
              .find({
                faction_presence: {
                  $elemMatch: { system_name_lower: message.StarSystem.toLowerCase() }
                }
            ...
            let allFactionsPresentInMessage = ebgsFactionsV4Model
              .find({
                name_lower: {
                  $in: messageFactionsLower
                }
            ...
            try {
              ...
              for (let factionNameLower of toRemove) {
                for (let factionObject of factionsPresentInSystemDB) {
                  if (
                    factionNameLower === factionObject.name_lower &&
                    factionObject.updated_at < new Date(message.timestamp)
                  ) {
                    let factionPresence = []
                ...
                for (let messageFaction of message.Factions) {
                  if (messageFaction.Name.toLowerCase() === factionNameLower) {
                    ...
                    for (let conflict of message.Conflicts) {
                      if (
                        conflict.Faction1.Name.toLowerCase() === factionNameLower ||
              ...
              for (let dbFaction of factionsAllDetails) {
                ...
                for (let messageFaction of message.Factions) {
                  ...
                  if (
                    messageFaction.Name.toLowerCase() === dbFaction.name_lower &&
                    factionPresence.updated_at < new Date(message.timestamp)
                  ) {
    ...
    if (message.event === 'Docked' || (message.event === 'Location' && message.Docked)) {
      try {
        ...
        try {
          let station = await ebgsStationsV4Model
            .findOne({
          ...
          if (station) {
            ...
            if (station.updated_at < new Date(message.timestamp)) {
              ...
              if (
                ...
                let timeNow = Date.now()
                let stationHistory = await ebgsHistoryStationV4Model
                  .find({
  ...
  this.doFactionUpdate = async (messageFaction, dbFaction, message) => {
    ...
    if (message.Conflicts) {
      message.Conflicts.forEach((conflict) => {
        if (
          conflict.Faction1.Name.toLowerCase() === factionName ||
    ...
    for (let faction of dbFaction.faction_presence) {
      if (faction.system_name_lower === message.StarSystem.toLowerCase()) {
        if (
          ...
          faction.conflicts &&
          _.isEqual(
            _.sortBy(faction.conflicts, ['opponent_name_lower']),
          ...
          let timeNow = Date.now()
          let factionHistory = await ebgsHistoryFactionV4Model
            .find({
