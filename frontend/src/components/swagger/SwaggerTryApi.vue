<template>
  <v-dialog v-model="dialogState" fullscreen hide-overlay scrollable transition="dialog-bottom-transition">
    <v-card>
      ...
      <v-card-text>
        <v-form>
          <div
            v-for="(parameter, index) in parametersSelected"
            :key="parameter + index"
...
export default {
  ...
  computed: {
    ...
    tryApiUrl() {
      let url =
        this.selectedSpecDoc.schemes + '://' + this.selectedSpecDoc.host + this.selectedSpecDoc.basePath + this.path
  ...
  subscriptions() {
    return {
      tryApiResponse: this.go$.pipe(debounceTime(300)).pipe(
        switchMap(() => {
