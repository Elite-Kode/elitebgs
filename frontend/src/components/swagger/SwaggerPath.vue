          ...
          <tbody>
            <tr v-for="parameter in currentPath[1][method].parameters" :key="parameter.name">
              ...
              <td v-else-if="parameter.$ref">
                ...
                &#60;
                <router-link :to="getDefinitionPath(getDefinitionFromRef(parameter.$ref))">
              ...
              <td v-else-if="parameter.type && parameter.type === 'array' && parameter.items.$ref">
                ...
                &#60;
                <router-link :to="getDefinitionPath(getDefinitionFromRef(parameter.items.$ref))">
        ...
        <v-simple-table dense class="elevation-1">
          <tbody>
            <tr v-for="responseCode in getResponses(method)" :key="responseCode">
              ...
              <td
                v-else-if="getResponse(responseCode).schema.type && getResponse(responseCode).schema.type === 'integer'"
              >
                <a
                  href="https://developer.mozilla.org/en-US/docs/Glossary/number"
                  target="_blank"
              ...
              <td v-else-if="getResponse(responseCode).schema.$ref">
                ...
                &#60;
                <router-link :to="getDefinitionPath(getDefinitionFromRef(getResponse(responseCode).schema.$ref))">
              ...
              >
                ...
                &#60;
                <router-link :to="getDefinitionPath(getDefinitionFromRef(getResponse(responseCode).schema.items.$ref))">
...
export default {
  ...
  watch: {
    currentPath(newVal, oldVal) {
      if (
        (_isEmpty(oldVal) && !_isEmpty(newVal)) ||
        (!_isEmpty(oldVal) && !_isEmpty(newVal) && newVal[0] !== oldVal[0])
      ) {
        this.checkRedirect()
      }
    },
